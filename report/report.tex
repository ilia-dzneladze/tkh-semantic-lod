...
about 7\% of the affinity, not the 30\% its weight suggests.\par}
\vspace{5.5pt}
{\color{accent}\hrule height 0.6pt}
\end{minipage}
...
lower on average), so it didn't ship, though it came closer than I first reported.
...
The UPGMA implementation, against scipy & Blind judgements from one LLM rater, of the labeller's model family; no second rater, no human pass \\
...
and a different result elsewhere can be traced to the embeddings or to the code. A reproducer can
also swap in their own labels and blind ratings and rerun the whole evaluation on them.
